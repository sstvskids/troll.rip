...
function Library:save_flags()
    ...
    writefile(`troll.rip/configs/{game.GameId}.lua`, flags)
end

function Library:load_flags()
    if not isfile(`troll.rip/configs/{game.GameId}.lua`) then Library.save_flags() return end

    local flags = readfile(`troll.rip/configs/{game.GameId}.lua`)
